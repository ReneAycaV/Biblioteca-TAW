...
export class MultasService {
    ...
    private calcularDiasAtraso(fechaEsperada: Date, fechaActual: Date = new Date()): number {
        const hoySinHora = new Date(fechaActual.getFullYear(), fechaActual.getMonth(), fechaActual.getDate());
        const fechaEsperadaSinHora = new Date(fechaEsperada.getFullYear(), fechaEsperada.getMonth(), fechaEsperada.getDate());
    ...
    private mapearEstadoLibro(estadoLibro: EstadoLibroDevuelto | null): string {
        ...
        return estadoLibro === EstadoLibroDevuelto.DEVUELTO_BUENO ? 'devuelto_bueno' : 'perdido_total';
    }

    private calcularMontoEnVivo(prestamo: PrestamoEntity, multa: MultaEntity): { monto: number | null, estadoLibro: EstadoLibroDevuelto | null, requiereDecision: boolean } {
    ...
    async actualizarMultaPorVencimiento(prestamo: PrestamoEntity): Promise<void> {
        const diasAtraso = this.calcularDiasAtraso(prestamo.fechaDevolucionEsperada);
        ...
        if (prestamo.multa) {
            ...
            if (prestamo.multa.diasAtraso !== diasAtraso) {
                ...
                console.log(`Multa ${prestamo.multa.idMulta} actualizada: ${diasAtraso} días - $${nuevoMonto}`);
            }
        } else {
            // Crear nueva multa
            ...
            console.log(`Multa generada para préstamo ${prestamo.idPrestamo}: ${diasAtraso} días - $${nuevoMonto}`);
        }
    }

    async pagarMulta(idMulta: number, userId: number, condiciones?: 'BUENO' | 'PERDIDO'): Promise<any> {
        const prestamo = await this.prestamosRepository.findOne({
            where: { multa: { idMulta: idMulta } },
            relations: ['usuario', 'multa', 'libro']
        });

        if (!prestamo) throw new NotFoundException('No se encontró un préstamo asociado a esta multa');
        ...
        if (prestamo.usuario.idUsuario !== userId) {
            throw new ForbiddenException('No puedes pagar una multa que no te pertenece');
        }
        ...
        if (prestamo.estado === EstadoPrestamo.PERDIDO || condiciones === 'PERDIDO') {
